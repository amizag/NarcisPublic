...
def Direct_Move_mirror_high(mirrorNum, Height, Beta, Gama, Duration):
    # pass
    R = 58; # servo arm length
    Hcst = 27.0; # The height constante. 27mm here
    Speed = 10
...
Direct_Move_mirror_high(0, MaxArmHeight, 0, 0, 0)
time.sleep(2)
...
while True:
    ...
    if len(faces) == 0:
        ...
        if NoFaceCount > 50:
            ...
            if ActualPositionBeta < 0:
                ActualPositionBeta = ActualPositionBeta + 0.5
            if ActualPositionBeta > 0:
                ActualPositionBeta = ActualPositionBeta - 0.5   
            if ActualPositionGama < 0:
                ActualPositionGama = ActualPositionGama + 0.5
            if ActualPositionGama > 0:
                ActualPositionGama = ActualPositionGama - 0.5               
            Direct_Move_mirror_high(0, MaxArmHeight, -ActualPositionBeta, -ActualPositionGama, 0)
...
#             kit.servo[2].angle = MIN2+40
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.circle   (frame, (x+ww,y+hh), radius=2, color=(0, 0, 255), thickness=-1)
        # cv2.circle   (frame, (430,240), radius=2, color=(0, 0, 255), thickness=-1)
        ...
        zc = float(x -(FRAME_W/2))
        tc = float(y - (FRAME_H/2))
        Dist = zc**2+tc**2
        ...
        FaceIndex += 1   
        # time.sleep(0.03)                    #delay one second
        # break
    

    if len(faces) > 0:

        ind = np.argsort(DistArray)
        z = zcArray[ind[0]]
        t = tcArray[ind[0]]
        ...
        Direct_Move_mirror_high(0, MaxArmHeight,   - AbsPositionBeta,  -AbsPositionGama, 0.00 )
    ...
    cv2.line     (frame, (0,120), (430,120), color=(0, 0, 255), thickness=1)
    cv2.line     (frame, (215,0), (215,240), color=(0, 0, 255), thickness=1)    
